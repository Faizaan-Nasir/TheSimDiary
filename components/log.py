...
class logBook(QWidget):
    ...
    def showUI(self):
        ...
        self.heading.setStyleSheet(
            "background:rgba(255, 255, 255, 0.725);font-size:45px;font-weight:800;padding:8px;margin-left:50px;border-radius:10px;color:#515151"
        )

        self.goBack = QPushButton("< Go Back", parent=self)
        self.goBack.move(80, 57)
        self.goBack.setStyleSheet(
            "background:transparent;font-size:18px;font-weight:800;color:#515151"
        )
        ...
        self.background.setStyleSheet(
            "QWidget { background: rgba(255, 255, 255, 0.725); border-radius: 10px; }"
        )
        ...
        try:
            self.flight = Flight(
                self.data[
                    self.data.callsign
                    == self.df.callsign.loc[self.df.callsign.count() - 1]
                ].iloc[0],
                self.airports,
                self.aircrafts,
            ...
            self.flight = Flight(
                self.temp.iloc[0], self.airports, self.aircrafts, parent=self.background
            )
        self.tableArea = QWidget(self)
        self.tableArea.setFixedSize(885, 595)
        self.tableArea.setStyleSheet(
            "QWidget { background: rgba(255, 255, 255, 0.725); border-radius: 10px; }"
        )
        ...
        self.date.setStyleSheet(
            "background:rgba(255, 255, 255, 0);font-size:18px;font-weight:800;color:#515151;border-style: solid;border-radius:0px;border-width: 0px 1px 2px 0px;border-color:#515151;"
        )
        ...
        self.cs.move(737, 1)
        self.cs.setStyleSheet(
            "background:rgba(255, 255, 255, 0);font-size:18px;font-weight:800;color:#515151;border-style: solid;border-radius:0px;border-width: 0px 0px 2px 1px;border-color:#515151;"
        )
        self.cs.setAlignment(QtCore.Qt.AlignCenter)

        self.searchArea = QWidget(self)
        self.searchArea.setFixedSize(400, 280)
        self.searchArea.setStyleSheet(
            "QWidget { background: rgba(255, 255, 255, 0.725); border-radius: 10px; }"
        )
        self.searchArea.move(50, 440)

        self.searchCall = QLineEdit(parent=self.searchArea)
        self.searchCall.setStyleSheet(
            "font-size:20px;border:none;background:#A4A4A4;border-radius:10px;padding:10px;color:white;font-weight:400;"
        )
        ...
        self.searchStatus.setStyleSheet(
            "font-size:20px;border:none;background:#A4A4A4;border-radius:10px;padding:10px;color:white;font-weight:400;"
        )
    ...
    def submitRecord(self):
        try:
            ...
            self.exception = QMessageBox.critical(
                self.addRec, "Error!", "Incorrect entry in one or more field(s)."
            )
        finally:
    ...
    def allFlights(self):
        ...
        try:
            ...
            self.flight = Flight(
                df.iloc[0], self.airports, self.aircrafts, parent=self.background
            )
        self.flight.show()
    ...
    def byStatus(self):
        try:
            ...
            self.table.deleteLater()
            self.table = Table(
                self.df[(self.df.status == self.searchStatus.text())],
                parent=self.tableArea,
            )
            self.table.show()
    ...
    def searchCallsign(self):
        try:
            ...
            self.flight.deleteLater()
            self.flight = Flight(
                self.df[self.df.callsign == searchCall].iloc[0],
                self.airports,
                self.aircrafts,
                parent=self.background,
            )
            ...
            self.table.deleteLater()
            self.table = Table(
                self.df[self.df.callsign == searchCall], parent=self.tableArea
            )
            self.table.show()
    ...
    def byMod(self):
        try:
            ...
            self.table.deleteLater()
            self.table = Table(
                self.df[self.df["mod"] == self.searchMod.text()], parent=self.tableArea
            )
            self.table.show()
...
class Table(QTableWidget):
    def __init__(self, df, **kwargs):
        ...
        self.setStyleSheet(
            """QWidget { background: rgba(255, 255, 255, 0); border-radius: 10px; }
                                    QTableWidget::item {
        ...
        for i in range(self.df.callsign.count() - 1, -1, -1):
            ...
            self.date.setStyleSheet(
                "background:rgba(255, 255, 255, 0);font-size:15px;font-weight:400;color:#515151;border-right:1px solid #515151;border-radius:0px;"
            )
            self.date.setAlignment(QtCore.Qt.AlignCenter)
            ...
            self.cs.setStyleSheet(
                "background:rgba(255, 255, 255, 0);font-size:15px;font-weight:400;color:#515151;border-left:1px solid #515151;border-radius:0px;"
            )
            self.cs.setAlignment(QtCore.Qt.AlignCenter)
